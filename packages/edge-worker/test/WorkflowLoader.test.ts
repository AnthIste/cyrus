...
describe("WorkflowLoader - tilde expansion", () => {
	let tempDir: string;
	let parser: WorkflowParser;

	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "workflow-loader-tilde-"));
		const schemaPath = path.join(
			__dirname,
			"../src/workflows/workflow-schema.json",
		);
	...
	/**
	 * Helper to create a workflow structure in a directory
	 */
	function createWorkflowStructure(baseDir: string): {
		workflowsDir: string;
		promptsDir: string;
	...
	it("should expand tilde in source path", async () => {
		// Create a directory in the temp folder that we'll reference with a tilde-style path
		// We'll simulate a tilde path by creating a symlink from the home directory
		const { workflowsDir } = createWorkflowStructure(tempDir);
		createWorkflowFile(workflowsDir, "workflow.yaml", [
			{ name: "tilde-test", description: "Tilde expansion test" },
		]);

		// Create a symlink in home directory pointing to our temp dir
		const homeDir = os.homedir();
		const symlinkName = `workflow-loader-test-${Date.now()}`;
		const symlinkPath = path.join(homeDir, symlinkName);

		try {
			// Create a symlink from home directory to our temp dir
			fs.symlinkSync(tempDir, symlinkPath);

			// Use tilde path to reference the symlink
			const loader = new WorkflowLoader(
				{ source: `~/${symlinkName}`, path: "workflows/" },
				parser,
			);

			// Verify the path is expanded correctly
			const workflowPath = loader.getWorkflowPath();
			expect(workflowPath).toBe(path.join(homeDir, symlinkName, "workflows/"));
			expect(workflowPath).not.toContain("~");

			// Verify workflows can be loaded
			const procedures = await loader.load();
			expect(procedures.size).toBe(1);
			expect(procedures.has("tilde-test")).toBe(true);
		} finally {
			// Cleanup symlink
			try {
	...
	it("should expand bare tilde (~) to home directory", () => {
		const loader = new WorkflowLoader({ source: "~", path: "workflows/" });
		const workflowPath = loader.getWorkflowPath();
		expect(workflowPath).toBe(path.join(os.homedir(), "workflows/"));
		expect(workflowPath).not.toContain("~");
	});

	it("should not affect absolute paths", () => {
		const loader = new WorkflowLoader({
			source: "/absolute/path",
			path: "workflows/",
		});
		const workflowPath = loader.getWorkflowPath();
		expect(workflowPath).toBe("/absolute/path/workflows/");
	});

	it("should not affect relative paths without tilde", () => {
		const loader = new WorkflowLoader({
			source: "relative/path",
			path: "workflows/",
		});
		const workflowPath = loader.getWorkflowPath();
		expect(workflowPath).toBe("relative/path/workflows/");
	});

	it("should handle tilde with nested path", () => {
		const loader = new WorkflowLoader({
			source: "~/nested/path/to/workflows",
			path: "workflows/",
		});
		const workflowPath = loader.getWorkflowPath();
		expect(workflowPath).toBe(
			path.join(os.homedir(), "nested/path/to/workflows", "workflows/"),
		);
	});

	it("should show expanded path in error messages", async () => {
		const loader = new WorkflowLoader({
		...
		// Error message should show expanded path, not the tilde
		expect(errors._directory).toContain(os.homedir());
		expect(errors._directory).not.toContain("~/");
	});
});
