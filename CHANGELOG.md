...
### Fixed
- **Tilde expansion in workflows repository path** - Fixed an issue where tilde (`~`) paths in `workflowsRepository.source` configuration were not expanded to the user's home directory, causing workflow directories to not be found. ([RUB-76](https://linear.app/rbakker/issue/RUB-76), [#10](https://github.com/AnthIste/cyrus/pull/10))
