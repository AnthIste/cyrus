...
### Fixed
- **Self-auth works with empty repositories** - Fixed an issue where new users couldn't authenticate with Linear when their config had an empty repositories array. Credentials are now stored at the workspace level, allowing `cyrus self-add-repo` to work immediately after `cyrus self-auth`. ([RUB-57](https://linear.app/rbakker/issue/RUB-57), [#2](https://github.com/AnthIste/cyrus/pull/2))
